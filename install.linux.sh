...
if [ -x "$TPM_INSTALL" ]; then
  ...
  log_skip "tpm not installed yet"
fi

# tmux-fingers' own install wizard is interactive and fetches through the
# GitHub API, which is rate-limited to failure from this network's shared NAT
FINGERS_BIN="$XDG_CONFIG_HOME/tmux/plugins/tmux-fingers/bin/tmux-fingers"
if [ -d "$XDG_CONFIG_HOME/tmux/plugins/tmux-fingers" ] && [ ! -x "$FINGERS_BIN" ]; then
  fingers_ver="$(curl -sI https://github.com/Morantron/tmux-fingers/releases/latest | grep -i '^location' | grep -oE 'tag/[^[:space:]]+' | cut -d/ -f2 | tr -d '\r')"
  if [ -n "$fingers_ver" ]; then
    mkdir -p "${FINGERS_BIN%/*}"
    curl -sL -o "$FINGERS_BIN" "https://github.com/Morantron/tmux-fingers/releases/download/${fingers_ver}/tmux-fingers-${fingers_ver#v}-linux-x86_64" \
      && chmod +x "$FINGERS_BIN" \
      && log_done "tmux-fingers binary ${fingers_ver}"
  else
    log_skip "tmux-fingers binary: could not resolve latest release"
  fi
fi
...
if [[ -n "${WSL_DISTRO_NAME:-}" ]] && command -v winget.exe >/dev/null 2>&1; then
  ...
  if ! powershell.exe -NoProfile -Command \
    ...
    echo "  Open Windows Terminal as Administrator, start this WSL" >&2
  ...
  log_step "Flow Launcher: curated settings"
  powershell.exe -NoProfile -ExecutionPolicy Bypass \
  ...
  if [[ -n "$win_userprofile_raw" ]]; then
    ...
    powershell.exe -NoProfile -ExecutionPolicy Bypass \
      -File "$(wslpath -w "$DOTFILES_PATH/winget/nightlight-shortcut.ps1")" >/dev/null 2>&1 || true
    log_done "Start Menu: Night Light (launcher-indexable -> bluelightreduction toggle)"
