...
**New comments (write side):** KEEP and WRITE are not symmetric — Rule 4 protects comments that already exist in the file; it is not authority to mint one on code you are adding. For a comment the user did not ask for, only Rule 4's closed list qualifies: an observed failure that forces this exact shape, an upstream bug/issue reference, a required ordering, a non-obvious unit/format/encoding. Never self-written, even though "unrecoverable from the code": what an identifier is or refers to, why this key/shape was chosen over an alternative, background on the tool — commit message. Example from a real violation: on a line adding AccordD64 to an ignore list, writing "its window title is LCSKVM\<session-id\>, whose numeric suffix changes per session, so match the process not the title" — an identifier mapping plus choice rationale, minted unasked; both are commit-message content. Tiebreak, and it dominates Rule 4: the moment you are weighing or arguing whether a new comment earns its place, it doesn't — a one-line config addition gets no new comment.
